...
public class ProductModel {
	...
	public long add(ProductBean bean) throws Exception {
		...
		pstmt.setString(3, bean.getProductAmount());
		pstmt.setDate(4, new java.sql.Date(bean.getPurchaseDate().getTime()));
		pstmt.setString(5, bean.getProductCatagory());
	...
	public void update(ProductBean bean) throws Exception {
		...
		pstmt.setString(2, bean.getProductAmount());
		pstmt.setDate(3, new java.sql.Date(bean.getPurchaseDate().getTime()));
		pstmt.setString(4, bean.getProductCatagory());
	...
	public ProductBean findByPK(long pk) throws Exception {
		...
		while (rs.next()) {
			...
			bean.setProductAmount(rs.getString(3));
			bean.setPurchaseDate(rs.getDate(4));
			bean.setProductCatagory(rs.getString(5));
	...
	public List search(ProductBean bean, int pageNo, int pageSize) throws Exception {
		...
		if (bean != null) {
			...
			if (bean.getProductAmount() != null && bean.getProductAmount().length() > 0) {
				sql.append(" AND productAmount like '" + bean.getProductAmount() + "%'");
			}

			if (bean.getProductCatagory() != null && bean.getProductCatagory().length() > 0) {
				sql.append(" AND productCategory like '" + bean.getProductCatagory() + "%'");
			}
