...
public class ProductCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("productAmount"))) {
			request.setAttribute("productAmount", PropertyReader.getValue("error.require", "productAmmount"));
			pass = false;
		}
		...
		if (DataValidator.isNull(request.getParameter("productCatagory"))) {
			request.setAttribute("productCatagory", PropertyReader.getValue("error.require", "productCatagory"));
			pass = false;
		}
	...
	protected BaseBean populateBean(HttpServletRequest request) {
		...
		bean.setProductAmount(DataUtility.getString(request.getParameter("productAmount")));

		bean.setPurchaseDate(DataUtility.getDate(request.getParameter("purchaseDate")));

		bean.setProductCatagory(DataUtility.getString(request.getParameter("productCatagory")));
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id != 0 && id > 0) {
			...
			try {
				...
				System.out.println("productCatagory name =====> " + bean.getProductCatagory());
