...
<body>
	...
	<form action="<%=ORSView.JOBREQUIREMENT_LIST_CTL%>" method="post">
		<center>
			...
			<%
				List clist = (List) request.getAttribute("jlist");
			...
			<%--  &nbsp;	<label>Client:</font> 
						...
						<%
							...
							map.put("Rishendra", "Rishendra");
							map.put("Richa", "Richa");
							map.put("Riddhima", "Riddhima");

							String hlist = HTMLUtility.getList("client", String.valueOf(bean.getClient()), map);
						%> <%=hlist%>
						
						 --%>
						  &emsp; <label>clinet</font> :
					</label> <%=HTMLUtility.getList("client", String.valueOf(bean.getClient()), clist)%>
						</td>
